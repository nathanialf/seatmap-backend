...
class BookmarkHandlerTest {
    ...
    void testListBookmarks_ValidToken_ReturnsBookmarks() throws Exception {
        ...
        assertTrue(response.getBody().contains("\"maxAllowed\":50"));
    ...
    void testCreateBookmark_ValidRequest_CreatesBookmark() throws Exception {
        ...
        when(mockAuthService.validateToken("valid-token")).thenReturn(testUser);
        when(mockBookmarkRepository.countBookmarksByUserId(testUserId)).thenReturn(5);
    ...
    @Test
    void testCreateBookmark_ExceedsLimit_ReturnsBadRequest() throws Exception {
        // Arrange
        CreateBookmarkRequest request = new CreateBookmarkRequest();
        request.setTitle("My Test Flight");
        ...
        when(mockAuthService.validateToken("valid-token")).thenReturn(testUser);
        when(mockBookmarkRepository.countBookmarksByUserId(testUserId)).thenReturn(50);
        ...
        assertTrue(response.getBody().contains("Maximum number of bookmarks reached (50)"));
        verify(mockAuthService).validateToken("valid-token");
        verify(mockBookmarkRepository).countBookmarksByUserId(testUserId);
